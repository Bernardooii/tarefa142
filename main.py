...
def popular_articles():
    ...
    for index,row in output.iterrows:
        dados = {
            ...
            'text': row['text'],
            'lang': row['lang'],
            'totalev': row['total_events']
        }
        popular.append(dados)
...
def recommended_articles():
    ...
    colunas=['url', 'title', 'text', 'lang', 'total_events']
    recomendado = pd.DataFrame(columns=colunas)
    
    for article in liked_articles:
        output = get_recommendations(article["contentId"])
        recomendado=recomendado.append(output)

    recomendado.drop_duplicates(inplace=True)

    recomendado = []

    for index, row in recomendado.iterrows():
        dados = {
            "url": row['url'],
            "title": row['title'],
            "text": row['text'],
